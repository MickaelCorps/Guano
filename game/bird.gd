...
func shoot_control(s, shoot_order):
	var lv = s.get_linear_velocity()
	var step = s.get_step()

	if shoot_order and not cur_shit_cooldown:
...
func _integrate_forces(s):
	...
	shoot_control(s, shoot)

	if cur_stun_cooldown:
		cur_stun_cooldown -= step
		if cur_stun_cooldown <= 0:
			cur_stun_cooldown = 0
			if not stalling:
				self.set_mode(MODE_CHARACTER)
	else:
		if move_right:
